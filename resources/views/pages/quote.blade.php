    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        @if(session('success'))
            <div class="mb-8 bg-meadow/10 border border-meadow/20 rounded-xl p-6 text-center">
                ...
                <h2 class="text-xl font-semibold text-alpine-green">Quote Request Submitted!</h2>
        ...
        <form method="POST" action="{{ route('quote.store') }}" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="full_name" id="full_name" value="{{ old('full_name') }}" required
                            class="w-full rounded-lg border border-stone/30 px-4 py-2.5 text-sm focus:border-alpine-green focus:ring-1 focus:ring-alpine-green">
                    </div>
                    <div>
                    ...
                    @foreach($serviceOptions as $value => $label)
                        <label class="flex items-center space-x-2 bg-snow rounded-lg p-3 cursor-pointer hover:bg-meadow/5 transition border border-stone/10">
                            <input type="checkbox" name="services_needed[]" value="{{ $value }}"
                                ...
                                class="rounded border-stone/30 text-alpine-green focus:ring-alpine-green">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea name="additional_notes" id="additional_notes" rows="3"
                            ...
                            class="w-full rounded-lg border border-stone/30 px-4 py-2.5 text-sm focus:border-alpine-green focus:ring-1 focus:ring-alpine-green">{{ old('additional_notes') }}</textarea>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
            ...
            <div class="text-center">
                <button type="submit" class="bg-alpine-green hover:bg-forest text-white px-10 py-4 rounded-lg font-bold text-lg transition shadow-lg">
