@extends('layouts.app')
@section('title', 'Moving Blog | Tips & Guides | Alpine Movers Calgary')
@section('meta_description', 'Expert moving tips, guides, and local Calgary advice from the Alpine Movers team.')
@section('schema')
            ...
            @forelse($posts as $post)
                <a href="{{ route('blog.show', $post) }}" class="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md border border-stone/10 transition">
                    ...
                    <div class="p-5">
                        ...
                        <h2 class="mt-2 text-lg font-semibold text-slate-dark group-hover:text-alpine-green transition">{{ $post->title }}</h2>
                        @if($post->excerpt)
