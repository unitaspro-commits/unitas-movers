@extends('layouts.app')
@section('title', 'Moving Services Calgary | Unitas Movers')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($services as $service)
                <a href="{{ route('services.show', $service) }}" class="group bg-white rounded-xl p-6 shadow-sm hover:shadow-md border border-stone/10 transition">
                    <h2 class="text-lg font-semibold text-slate-dark group-hover:text-unitas-green transition">{{ $service->name }}</h2>
                    <p class="mt-2 text-stone text-sm">{{ $service->meta_description }}</p>
                    <span class="mt-4 inline-block text-unitas-green text-sm font-semibold">Learn More &rarr;</span>
                </a>
            @endforeach
        </div>
    </div>
